    ...
    public static List<ErrorDto> byBindingResult(BindingResult bindingResult, MessageSourceHandler ms){
        ...
        if(bindingResult.hasErrors()){
            bindingResult.getGlobalErrors().stream()
                    .forEach(o -> errorList.add(new ErrorDto(o.getObjectName(), o.getCodes()[0], ms.getMessage(o.getCodes()[0],o.getArguments()))));
            List<ErrorDto> fieldErrorList = new ArrayList<ErrorDto>();
            bindingResult.getFieldErrors().stream()
                    .forEach(f -> fieldErrorList.add(new ErrorDto(f.getObjectName(), f.getField(),f.getCodes()[0], ms.getMessage(f.getCodes()[0],f.getArguments()))));
